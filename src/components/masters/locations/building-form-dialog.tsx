...
export function BuildingFormDialog({ open, onOpenChange, onSuccess, initialData }: any) {
  const [loading, setLoading] = useState(false);
  const [companies, setCompanies] = useState<any[]>([]);
  const isEdit = !!initialData;

  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_BASE_PATH || ""}/api/masters/companies`).then(r => r.json()).then(d => setCompanies(d.data || []));
  }, []);
  ...
  const onSubmit = async (values: any) => {
    ...
    try {
      ...
      toast.success(`Building ${isEdit ? "updated" : "created"} successfully`);
      onSuccess();
    } catch (error: any) {
