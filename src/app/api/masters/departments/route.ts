...
export async function POST(req: NextRequest) {
  try {
    ...
    const newItem = await prisma.department.create({ data: validatedData });
    await createAuditLog({ userId: session.user.id, action: AUDIT_ACTIONS.CREATE, module: AUDIT_MODULES.MASTER_DATA, details: `Created department ${newItem.code}`, entityId: newItem.id });
