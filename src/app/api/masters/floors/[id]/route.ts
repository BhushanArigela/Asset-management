...
export async function GET(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    const item = await prisma.floor.findUnique({ where: { id: params.id }, include: { building: true } });
...
export async function PUT(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const updated = await prisma.floor.update({ where: { id: params.id }, data });
...
export async function DELETE(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const deleted = await prisma.floor.update({ where: { id: params.id }, data: { isActive: false } });
