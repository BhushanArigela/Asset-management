...
export async function GET(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    const item = await prisma.assetCategory.findUnique({ where: { id: params.id } });
...
export async function PUT(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const updated = await prisma.assetCategory.update({ where: { id: params.id }, data });
...
export async function DELETE(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const deleted = await prisma.assetCategory.update({ where: { id: params.id }, data: { isActive: false } });
