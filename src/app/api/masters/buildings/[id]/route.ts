...
export async function GET(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    const item = await prisma.building.findUnique({ where: { id: params.id }, include: { company: true } });
...
export async function DELETE(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const deleted = await prisma.building.update({ where: { id: params.id }, data: { isActive: false } });
