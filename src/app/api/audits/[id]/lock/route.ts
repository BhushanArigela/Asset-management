...
export async function POST(req: Request, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (audit.status !== "COMPLETED") return new NextResponse("Only COMPLETED audits can be locked", { status: 400 });
    ...
    await prisma.auditLog.create({
      data: {
        ...
        remarks: "Audit locked"
      }
    });

    return NextResponse.json(updatedAudit);
  } catch (error) {
    console.error("[AUDIT_LOCK_POST]", error);
